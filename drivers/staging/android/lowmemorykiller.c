...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(p) {
		...
		{
		/*
			lowmem_print(2, "select %d (%s), adj %d, task_state %lu, skipped\n",
				     p->pid, p->comm, oom_adj, task_state );
		*/
			continue;
		}
		...
		/*
		lowmem_print(2, "select %d (%s), adj %d, task_state %lu, task_state_chr = %c, to kill\n",
			     p->pid, p->comm, oom_adj, task_state, stat_nam[task_state]);
		*/
	}
	if (selected) {
