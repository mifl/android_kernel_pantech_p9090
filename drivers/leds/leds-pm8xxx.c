...
///////////////////////////////////////////// p11309
static struct pm8xxx_led_data *led_ioctl;
/////////////////////////////////////////////
...
#if defined(CONFIG_MACH_MSM8960_STARQ) //p11309, // dhyang	
	Upper_Sub=(cmd & 0x0F);
	dbg("leds_fops_ioctl Upper_Sub : %d\n",Upper_Sub);
	led_kp_set(led_ioctl, level);
#else 

	led_ioctl->cdev.brightness = level << 4;
	led_ioctl->pwm_duty_cycles->duty_pcts[0] = 100;
	led_ioctl->pwm_duty_cycles->duty_pcts[1] = 100;
	led_ioctl->pwm_duty_cycles->start_idx = 0;
	led_ioctl->pwm_duty_cycles->num_duty_pcts = 2;

	pm8xxx_pwm_lut_config(
        led_ioctl->pwm_dev,
		led_ioctl->pwm_period_us,
        led_ioctl->pwm_duty_cycles->duty_pcts,
		led_ioctl->pwm_duty_cycles->duty_ms,
		led_ioctl->pwm_duty_cycles->start_idx,
		led_ioctl->pwm_duty_cycles->num_duty_pcts,
		0, 0,
		PM8XXX_LED_PWM_FLAGS );

	pm8xxx_pwm_lut_enable(led_ioctl->pwm_dev, led_ioctl->cdev.brightness);
...
static void led_lc_set(struct pm8xxx_led_data *led, enum led_brightness value)
{
	...
	dbg("[LED: led_lc_set] value = %d, level=%d, reg=%d \n", value, level, led->reg);

	rc = pm8xxx_writeb(led->dev->parent, SSBI_REG_ADDR_LED_CTRL(offset), led->reg);
	//rc = pm8xxx_writeb(led->dev->parent, SSBI_REG_ADDR_LED_CTRL(offset), 255);
	if (rc) {
		dev_err(led->cdev.dev, "can't set (%d) led value rc=%d\n", led->id, rc);
	}	
}
...
static int pm8xxx_led_pwm_work(struct pm8xxx_led_data *led)
{
	...
	dbg("pm8xxx_led_pwm_work : id=%d, level=%d\n", led->id, led->cdev.brightness);
...
#if defined(CONFIG_MACH_MSM8960_SIRIUSLTE)	//	p11309
		if(led_dat->id == PM8XXX_ID_LED_0) {
			led_ioctl = &led[i];			
		}
#elif defined(CONFIG_MACH_MSM8960_STARQ)	//	mirinae
		if(led_dat->id == PM8XXX_ID_LED_KB_LIGHT) {
			led_ioctl = &led[i];
		}
#endif
