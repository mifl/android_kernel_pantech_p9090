...
#ifdef CONFIG_OV8820_ACT//for reduce Actuator machine noise
    if (a_ctrl->curr_step_pos != 0) {
        rc = a_ctrl->func_tbl.actuator_move_focus(a_ctrl, MOVE_FAR, a_ctrl->curr_step_pos/5); //07_13_bsy add
        //printk("%s [QCTK] Move to half of far position, a_ctrl->curr_step_pos/2 = %d,  rc = %d\n", __func__, (a_ctrl->curr_step_pos)/4 , rc);
        mdelay(10);
        rc = a_ctrl->func_tbl.actuator_move_focus(a_ctrl, MOVE_FAR, a_ctrl->curr_step_pos - (a_ctrl->curr_step_pos)/5); //07_13_bsy add
        //printk("%s rc = %d\n", __func__, rc); // 07_13_bsy add
        mdelay(10);
        rc = a_ctrl->func_tbl.actuator_move_focus(a_ctrl, MOVE_FAR, a_ctrl->curr_step_pos - (a_ctrl->curr_step_pos)/5); //07_13_bsy add
        //printk("%s rc = %d\n", __func__, rc); // 07_13_bsy add
        mdelay(10);
