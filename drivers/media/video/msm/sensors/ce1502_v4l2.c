...
#define CE1502_FW_MINOR_VER	18
#define CE1502_PRM_MAJOR_VER	18
#define CE1502_PRM_MINOR_VER	1
#endif
...
static int32_t ce1502_set_coninuous_af(struct msm_sensor_ctrl_t *s_ctrl ,int8_t caf)
{
...
static int32_t ce1502_lens_stability(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    ce1502_set_coninuous_af(s_ctrl, 0); // AF-T stop	
    data_buf[0] = 0x01;
    data_buf[1] = 0x00;
    data_buf[2] = 0x00;
...
static int32_t ce1502_update_fw_boot(struct msm_sensor_ctrl_t *s_ctrl, const struct msm_camera_sensor_info *info)
{
	...
	if (rc < 0)
	{
            ...
            goto update_fw_boot_done;             
#else
		if(ce1502_saddr == 0x3C)
			ce1502_saddr = 0x3E;
		else
			ce1502_saddr = 0x3C;
		
		rc = ce1502_cmd(ce1502_saddr, 0xF0, NULL, 0);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			goto update_fw_boot_done; 
...
static int32_t ce1502_set_object_tracking(struct msm_sensor_ctrl_t *s_ctrl, int32_t ojt)
{
    ...
    {        
        ...
        rc = ce1502_set_coninuous_af(s_ctrl, 1);
#endif
...
static int32_t ce1502_sensor_set_focus_mode(struct msm_sensor_ctrl_t *s_ctrl, int8_t focusmode)
{
	...
	rc = ce1502_set_coninuous_af(s_ctrl, caf);
	if (rc < 0)
	{
		SKYCERR("ERR:%s ce1502_set_coninuous_af FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	}	
...
preview_ok:
    ...
    prev_caf2 = caf_b_ojt;
    ce1502_set_coninuous_af(s_ctrl, continuous_af_mode);
    caf_b_ojt = prev_caf2;
...
preview_ok:
    ...
    ce1502_set_coninuous_af(s_ctrl, continuous_af_mode);//continuous_af_mode
    caf_b_ojt = prev_caf2;
...
static int32_t ce1502_snapshot_config(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    prev_caf2 = caf_b_ojt;
    rc = ce1502_set_coninuous_af(s_ctrl, 0);
...
//        rc = ce1502_set_object_tracking(s_ctrl, ojt);
        ...
        rc = ce1502_set_coninuous_af(s_ctrl, caf_b_ojt);
        if(caf_b_ojt == 0)
        {
            data_buf[0] = 0x00;
            rc = ce1502_cmd(s_ctrl, 0x20, data_buf, 1); //set focus mode normal
...
//	.sensor_probe = ce1502_sensor_probe,
    ...
    .sensor_set_coninuous_af = ce1502_set_coninuous_af,
...
static struct msm_sensor_reg_t ce1502_regs = {
	...
	.num_conf = ARRAY_SIZE(ce1502_cid_cfg),
	//.num_conf = 2,
};
