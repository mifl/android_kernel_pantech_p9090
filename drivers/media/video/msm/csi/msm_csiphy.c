...
//#define QDBG_KELLY_TEST   // 2012.06.25 - check for result of csiphy reset
#ifdef QDBG_KELLY_TEST
#define DBG_CSIPHY 1
#else
#define DBG_CSIPHY 0
#endif
...
#ifdef QDBG_KELLY_TEST
void QCT_CSI_REG_Read(struct csiphy_device *csiphy_dev);
#endif
...
void QCT_CSI_REG_Read(struct csiphy_device *csiphy_dev)
{
	...
	for (i = 0; i < 4; i++)
	{
		data = msm_io_r(csiphy_dev->base + MIPI_CSIPHY_LNn_CFG2_ADDR + 0x40*i);
              pr_err("%s: %08x %08x\n", __func__, (int)(csiphy_dev->base + MIPI_CSIPHY_LNn_CFG2_ADDR + 0x40*i), (data));

	}

	data = msm_io_r(csiphy_dev->base + MIPI_CSIPHY_LNCK_CFG2_ADDR);
       pr_err("%s: %08x %08x\n", __func__, (int) (csiphy_dev->base + MIPI_CSIPHY_LNCK_CFG2_ADDR), (data));
	data = msm_io_r(csiphy_dev->base + MIPI_CSIPHY_GLBL_PWR_CFG_ADDR);
       pr_err("%s: %08x %08x\n", __func__, (int) (csiphy_dev->base + MIPI_CSIPHY_GLBL_PWR_CFG_ADDR), (data));
}
#endif
