										...
										const U8 *payload)
{
	...
	case 384: case 96: case 128: case 192:
		if ((payload[0] == 0x05) && (payload[1] == 0x00) && (payload[2] == 0xE0))
			hdr->fic_normal_sync_data = TRUE;
		else
			hdr->fic_normal_sync_data = FALSE;
		break;
	default:
										...
										const U8 *payload_ptr)
{
	...
	return verify_header(hdr, hdr_buf_ptr, payload_ptr);
}
...
{
	UINT i, k, diff;
	U8 header_data[RTV_CIF_HEADER_SIZE + 3];
	PARSER_STATUS_TYPE status;

	//ASSERT((prev_len + curr_len) >= (RTV_CIF_HEADER_SIZE + 3));

	if (prev_len == 0) {
		if ((curr_data[0] == 0xE2) && (curr_data[1] == 0xE2))
			status = parsing_header(hdr, curr_data, &curr_data[RTV_CIF_HEADER_SIZE]);
		else
			return PARSER_STATUS__INVALID_SC;
	}
	else {
		if (prev_len >= (RTV_CIF_HEADER_SIZE + 3)) {
			if((prev_data[0] == 0xE2) && (prev_data[1] == 0xE2))
				status = parsing_header(hdr, prev_data, &prev_data[RTV_CIF_HEADER_SIZE]);
			else
				return PARSER_STATUS__INVALID_SC;
		}
		else {
			...
			if ((header_data[0] == 0xE2) && (header_data[1] == 0xE2)) {
				diff = (RTV_CIF_HEADER_SIZE + 3) - prev_len;
				...
				status = parsing_header(hdr, header_data, &header_data[RTV_CIF_HEADER_SIZE]);
			}
			else
				return PARSER_STATUS__INVALID_SC;
		}
		...
		if ((cifb->prev_len + cifb->curr_len) <= TSIF_COLLECT_BUF_SIZE) {
			...
			if (cifb->prev_len)
				memmove((void *)g_pbPrevBufAllocPtr,
						cifb->prev_data + cifb->prev_len, cifb->prev_len);
										...
										UINT pkt_offset)
{
	...
	if (hdr->fic_size) {
		if (pkt_offset < hdr->fic_size)
			return PARSER_STATUS__INVALID_HDR;
		
		if (hdr->fic_normal_sync_data == FALSE) {
			if ((hdr->cif_size - hdr->fic_size) == hdr->sum_subch_size) {
				UINT pkt_size = RTV_CIF_HEADER_SIZE + hdr->sum_subch_size;
				UINT padding_bytes = pkt_size % 188;
				if (padding_bytes)
					padding_bytes = 188 - padding_bytes;

				if ((pkt_size + padding_bytes) <= pkt_offset) {
					if (++g_nFicIC == 1)
						fic_ic();
	...
	if (hdr->sum_subch_size) {
		hold_msc_size = pkt_offset - hdr->fic_size;
#if (RTV_NUM_DAB_AVD_SERVICE == 1)
	...
	if (pkt_offset > (RTV_CIF_HEADER_SIZE + hdr->cif_size))
							...
							UINT *invalid_offset)
{
	...
	if ((cifb->prev_len + cifb->curr_len) >= (hdr->pkt_size + RTV_CIF_HEADER_SIZE + 3)) {
		...
		*invalid_offset = 0;
		...
		while (prev_len) {
			if (*prev_data == 0xE2/*1st SC*/) { /* To fast detecting of SC */
				...
				if ((prev_len + curr_len) >= (RTV_CIF_HEADER_SIZE + 3)) {
		...
		while (curr_len) {
			if (*curr_data == 0xE2/*1st SC*/) { /* To fast detecting of SC */
				...
				if (curr_len >= (RTV_CIF_HEADER_SIZE + 3)) {
...
byte_search_detected:
	...
	else if (pkt_offset < hdr->pkt_size) /* Must NOT equal! */
		return proc_truncated_packet(hdr, next_hdr, pkt_offset);
	else
		return PARSER_STATUS__INVALID_HDR;
}
	...
	while (cifb->prev_len) {
		if (g_fUsePrevDecodedHeader == FALSE) {
			if (*cifb->prev_data == 0xE2/*1st SC*/) { /* To fast detecting of SC */
				if ((cifb->prev_len + cifb->curr_len) >= (RTV_CIF_HEADER_SIZE + 3))
	...
	while (cifb->curr_len) {
		if (g_fUsePrevDecodedHeader == FALSE) {
			if (*cifb->curr_data == 0xE2/*1st SC*/) { /* To fast detecting of SC */
				if (cifb->curr_len >= (RTV_CIF_HEADER_SIZE + 3))
						...
						E_RTV_SERVICE_TYPE svc_type)
{
	...
	UINT _2nd_sb_offset, in_subch_size = subch_size;
		...
		while (frag->len) {
			if (frag_data_ptr[0] == 0x47) { /* 1st sb */
				...
				if (cifb->prev_len > _2nd_sb_offset) {
					...
					if (cifb->curr_data[_2nd_sb_offset - cifb->prev_len] == 0x47) /* 2nd sb */
						_2nd_sync_byte_detected = TRUE;
				}
			...
			if (cifb->prev_data[0] == 0x47) { /* 1st sb */
				if (subch_size > 188) {
					if (cifb->prev_len > 188) {
						...
						if (cifb->curr_data[188 - cifb->prev_len] == 0x47) /* 2nd sb */
							_2nd_sync_byte_detected = TRUE;
					}
				}
...
frag_subch_data:
	...
	return (in_subch_size - subch_size);
}
