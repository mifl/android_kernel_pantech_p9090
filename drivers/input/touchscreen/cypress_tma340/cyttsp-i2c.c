...
//	wcjeong - 2012.03.09 touch id disable
...
#define dbg(fmt, args...)   printk("[CYTTSP340]" fmt, ##args)
#else
#define dbg(fmt, args...)
#endif
...
int pantech_ctl_update(int cmd, int value)
{
	...
	switch(cmd)
	{
		...
		case ISSP_IOCTL_POWER:
 			...
 			if(value) gpio_set_value(GPIO_TOUCH_POWER, 1);
 			else	  gpio_set_value(GPIO_TOUCH_POWER, 0);
...
void cyttsp_xy_worker(struct work_struct *work)
{
	...
	do 
	{
// 		buf[0] = CYTTSP_REG_BASE;
// 		retval = i2c_master_send(ts->client, (const char *) buf, 1);		
// 		retval = i2c_master_recv(ts->client, (u8 *)&g_xy_data, sizeof(struct cyttsp_gen3_xydata_t));		
...
static int cyttsp_initialize(struct i2c_client *client, struct cyttsp *ts)
{
	...
	if(ts->client->irq == 0) {
		...
		error = request_irq (client->irq,cyttsp_irq,IRQF_TRIGGER_FALLING, client->dev.driver->name,ts);
		if (error) {
