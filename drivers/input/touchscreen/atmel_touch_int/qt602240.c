...
void qt_delta_check_func(struct work_struct * p) {
	//mutex_lock(&qt602240_data->lock);
	dbg_CR("[touch] qt_delta_check_func & f_delta_check is true\n");
	f_delta_check=true;	
	//mutex_unlock(&qt602240_data->lock);
}
#endif // ANTI_TOUCH_RECOVERY_AFTER_PROTECTION_MODE
...
void  clear_event(uint8_t clear)
{
	...
	for ( i= 0; i<MAX_NUM_FINGER; i++ )
	{
		if((fingerInfo[i].mode == TSC_EVENT_MENU) || (fingerInfo[i].mode == TSC_EVENT_HOME) || (fingerInfo[i].mode == TSC_EVENT_BACK) || (fingerInfo[i].mode == TSC_EVENT_SEARCH))  // p11223
		{
	...
	if (object_type == TOUCH_MULTITOUCHSCREEN_T9)
	{
		...
		if(quantum_msg[1] == 0x10) {
			...
			if(debugInfo.autocal_flag){																			// If touch auto calibration is enabled.
				dbg_CR("[TOUCH] Touch calibration is executed in auto calibration\n");
