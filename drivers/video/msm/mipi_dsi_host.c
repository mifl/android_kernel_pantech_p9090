...
#if defined(CONFIG_MACH_MSM8960_CHEETAH) || defined(CONFIG_MACH_MSM8960_VEGAPVW) || defined(CONFIG_MACH_MSM8960_SIRIUSLTE) || defined(CONFIG_MACH_MSM8960_MAGNUS)
	uint32 data;
#endif
	uint32 dsi_ctrl, intr_ctrl;
...
#if defined(CONFIG_MACH_MSM8960_VEGAPVW) || defined(CONFIG_MACH_MSM8960_SIRIUSLTE) || defined(CONFIG_MACH_MSM8960_EF44S) || defined(CONFIG_MACH_MSM8960_MAGNUS) /* LCD shinjg */
		wait_for_completion_timeout(&dsi_dma_comp, 5UL); // 10 -> 5 flick cursor
#else
		...
		struct dsi_buf *tp, struct dsi_cmd_desc *cmds, int cnt)
{
	...
	MIPI_OUTP(MIPI_DSI_BASE + 0x38, 0x10000000);    // High Speed for command Mode , shinjg
#endif
	/* turn on cmd mode
...
//20111213 ejkim lcd_display_off lock (enter o, eixt x) 
    mdelay(1);   
#endif    
