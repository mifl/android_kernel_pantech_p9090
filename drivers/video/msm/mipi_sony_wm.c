...
if (module_vender)
		mipi_dsi_cmds_tx(mfd, &sony_tx_buf, sony_display_init_ws20_cmds,
				ARRAY_SIZE(sony_display_init_ws20_cmds));
else
		mipi_dsi_cmds_tx(mfd, &sony_tx_buf, sony_display_init_ws10_cmds,
...
int bl_level;
 ...
 if (prev_bl_level == mfd->bl_level)
    ...
    wrdisbv[1] = bl_level;
    printk(KERN_ERR"mipi_sony_set_backlight bl_level =%d \n",bl_level);
#if 1	
    if(first_enable == 0)
    {
        gpio_set_value_cansleep(gpio24, GPIO_HIGH_VALUE);
    ...
    mipi_dsi_cmds_tx(mfd, &sony_tx_buf, sony_display_cabc_bl_set_cmds,
