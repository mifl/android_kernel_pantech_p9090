...
#define QUALCOMM_BUG_FIX_LCD_MDP_DOWNSCALING_BUG  /* case #788251 p13447 shinjg  20120312 */
/*AOT move blink, black display, rotate blink (CASE #859619, #866780)*/
#define FEATURE_QUALCOMM_BUG_FIX_LCD_AOT_DISPLAY_ERROR
/* Abnomal reset mdp4 driver (CASE #872600)*/
#define FEATURE_QUALCOMM_BUG_FIX_LCD_MDP4_ABNOMAL_RESET
#define FEATURE_QUALCOMM_BUG_FIX_LCD_OVERLAY_BLINK //20120615 shkwak, qcom patch for overlay blink
#define  FEATURE_QUALCOMM_BUG_FIX_LCD_MDP_ROTATOR_DOWNSCALE
...
void mdp4_overlay_dmae_cfg(struct msm_fb_data_type *mfd, int atv)
{
	...
	#ifdef CONFIG_FB_MSM_HDMI_MHL  // Temp build error fix
		mdp_vid_quant_set();
	#endif	
//1534 patch                MDP_OUTP(MDP_BASE + 0xb0070, 0xff0000);
//1534 patch                MDP_OUTP(MDP_BASE + 0xb0074, 0xff0000);
//1534 patch                MDP_OUTP(MDP_BASE + 0xb0078, 0xff0000);
	}
	...
	struct msm_fb_data_type *mfd, uint32 perf_level, uint32 pclk_rate)
{
	...
	{
	/*Do. layer1 add (!(ctrl->panel_mode & MDP4_PANEL_DTV)) to skip BLT when it is on HDMI*/ 
		 if(!(ctrl->panel_mode & MDP4_PANEL_DTV)&&(( mdp_pixels_produced <= (num_hsync_pix_clks + 50) ||
...
#ifdef QUALCOMM_BUG_FIX_LCD_MDP_DOWNSCALING_BUG
	else if (req->src.width*req->src.height <= 0x40000)
#else
...
#ifdef CONFIG_FB_MSM_DTV //20120315 yjw			
...
#ifdef FEATURE_QUALCOMM_BUG_FIX_LCD_MDP_ROTATOR_DOWNSCALE
	if(mfd->panel_info.type == MIPI_VIDEO_PANEL)
	{
	     use_blt = 0;
	}	
...
#ifdef QUALCOMM_BUG_FIX_LCD_MDP_DOWNSCALING_BUG
//        pr_err("[%s]perf_level=%d", __func__,perf_level);
#endif
