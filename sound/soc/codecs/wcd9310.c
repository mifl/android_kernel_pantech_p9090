...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
#define CFILT_ADJUST_MS_DEFAULT 10
#define CFILT_ADJUST_MS_LONG 200

static int cfilt_adjust_ms = CFILT_ADJUST_MS_DEFAULT;
...
static void __tabla_codec_switch_micbias(struct snd_soc_codec *codec,
					 ...
					 bool checkpolling, int cfilt_adj_ms)
#else
static void __tabla_codec_switch_micbias(struct snd_soc_codec *codec,
					 int vddio_switch, bool restartpolling,
					 bool checkpolling)
	...
	if (vddio_switch && !tabla->mbhc_micbias_switched &&
		...
		if (tabla->mbhc_data.micb_mv != VDDIO_MICBIAS_MV) {
			...
			snd_soc_update_bits(codec,
					tabla->mbhc_bias_regs.cfilt_val,
					0xFC, (cfilt_k_val << 2));
			usleep_range(cfilt_adj_ms * 1000,
					cfilt_adj_ms * 1000);
#else
...
{
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
	return __tabla_codec_switch_micbias(codec, vddio_switch, true, true, cfilt_adjust_ms);
#else
...
#if 0 //defined(T_EF44S) //[LS1 CHD DB16] workaround code for headset remove interrupt
...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
tabla_codec_get_plug_type(struct snd_soc_codec *codec, bool highhph, int cfilt_adj_ms)
#else
...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
			__tabla_codec_switch_micbias(tabla->codec, 1, false, false, cfilt_adj_ms);
#else
			__tabla_codec_switch_micbias(tabla->codec, 1, false, false);
#endif
...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
			__tabla_codec_switch_micbias(tabla->codec, 0, false, false, cfilt_adj_ms);
#else
			__tabla_codec_switch_micbias(tabla->codec, 0, false, false);
#endif
		} else {
			if (gndswitch)
...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
static int tabla_get_cfilt_adjust_long(void)
{
	return (cfilt_adjust_ms == CFILT_ADJUST_MS_DEFAULT ?
		CFILT_ADJUST_MS_LONG : cfilt_adjust_ms);
}
#endif
...
static void tabla_hs_correct_gpio_plug(struct work_struct *work)
{
	...
	cfilt_ms = tabla_get_cfilt_adjust_long();
#endif
	...
	while (!time_after(jiffies, timeout)) {
		...
		TABLA_ACQUIRE_LOCK(tabla->codec_resource_lock);
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
		plug_type = tabla_codec_get_plug_type(codec, true, cfilt_ms);
#else
...
static void tabla_codec_decide_gpio_plug(struct snd_soc_codec *codec)
{
	...
	tabla_turn_onoff_override(codec, true);
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
	plug_type = tabla_codec_get_plug_type(codec, true, cfilt_adjust_ms);
#else
	plug_type = tabla_codec_get_plug_type(codec, true);
#endif
	tabla_turn_onoff_override(codec, false);
...
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
	plug_type = tabla_codec_get_plug_type(codec, tabla->mbhc_cfg.gpio ? true : false, cfilt_adjust_ms);
#else
...
static irqreturn_t tabla_hs_remove_irq(int irq, void *data)
{
	...
	if (vddio)
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
		__tabla_codec_switch_micbias(priv->codec, 0, false, true, cfilt_adjust_ms);
#else
	...
	if (vddio)
#if defined(T_MAGNUS) || defined(T_VEGAPVW) // Qualcomm Workaround Patch
		__tabla_codec_switch_micbias(priv->codec, 1, true, true, cfilt_adjust_ms);
#else
