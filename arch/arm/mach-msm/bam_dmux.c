...
//20121015 cobo_model applied SBA for CR406946 about a2_taskq.c error fatal
...
//CR406946 FIX	while (rx_len_cached < NUM_BUFFERS) {
      while (bam_connection_is_active && rx_len_cached < NUM_BUFFERS) { //CR406946 FIX	

		if (in_global_reset)
			goto fail;

//CR406946 FIX			info = kmalloc(sizeof(struct rx_pkt_info), GFP_KERNEL);
		info = kmalloc(sizeof(struct rx_pkt_info), GFP_NOWAIT | __GFP_NOWARN); //CR406946 FIX
		if (!info) {
//CR406946 FIX			pr_err("%s: unable to alloc rx_pkt_info\n", __func__);
                    DMUX_LOG_KERR("%s: unable to alloc rx_pkt_info, will retry later\n",__func__); //CR406946 FIX
			goto fail;
		}

		INIT_WORK(&info->work, handle_bam_mux_cmd);

//CR406946 FIX		info->skb = __dev_alloc_skb(BUFFER_SIZE, GFP_KERNEL);
		info->skb = __dev_alloc_skb(BUFFER_SIZE, GFP_NOWAIT | __GFP_NOWARN); //CR406946 FIX

		if (info->skb == NULL) {
//CR406946 FIX			DMUX_LOG_KERR("%s: unable to alloc skb\n", __func__);
...
fail:
	if (rx_len_cached == 0) {
//CR406946 FIX		DMUX_LOG_KERR("%s: RX queue failure\n", __func__);
//CR406946 FIX		in_global_reset = 1;
	DMUX_LOG_KERR("%s: rescheduling\n", __func__); //CR406946 FIX
       schedule_delayed_work(&queue_rx_work, msecs_to_jiffies(100)); //CR406946 FIX
	}
}
...
static int bam_dmux_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&queue_rx_work, queue_rx_work_func); //CR406946 FIX
